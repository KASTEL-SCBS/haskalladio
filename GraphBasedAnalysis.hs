...
module GraphBadedAnalysis where
...
class AbstractDesignModel m => SecurityGraph m where
  data Node m = ParameterAssembly (Parameter m) (AssemblyContext m)
                | FullyAccessibleContainer (ResourceContainer m)
                | FullyAccessibleLink (LinkingResource m)
