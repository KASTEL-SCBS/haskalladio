...
fromGreiner :: (Ord p, Ord d) => Procedure p -> Specification p d -> Specification p p
...
fromHecker :: (Ord p, Ord d) => Procedure p -> Specification p d -> Specification p p
...
conversionEquivalent1 ::  (Ord p, Ord d) => Procedure p -> Implementation p -> Specification p d -> Bool
conversionEquivalent1 pr impl sp =  NIG.secure NIG.key pr impl sp ↔ NI.secure  NI.key pr impl sp'
  where sp' = fromGreiner pr sp

conversionEquivalent2 ::  (Ord p, Ord d) => Procedure p -> Implementation p -> Specification p d -> Bool
conversionEquivalent2 pr impl sp =  NI.secure NI.key pr impl sp ↔ NIG.secure  NIG.key pr impl sp'
  where sp' = fromHecker pr sp

conversionEquivalent3 ::  (Ord p, Ord d) => Procedure p -> Implementation p -> Specification p d -> Bool
conversionEquivalent3 pr impl sp =  NIG.secure NIG.joana pr impl sp ↔ NI.secure  NI.joana pr impl sp'
  where sp' = fromGreiner pr sp

conversionEquivalent4 ::  (Ord p, Ord d) => Procedure p -> Implementation p -> Specification p d -> Bool
